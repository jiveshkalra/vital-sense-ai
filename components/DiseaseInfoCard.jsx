...
import { AlertTriangle, Thermometer, Stethoscope, Shield, Info } from 'lucide-react'
          ...
          <div className="grid grid-cols-2 gap-2">
            {info.treatment.map((treatment, index) => (
              <motion.div 
                ...
                animate={{ opacity: 1, x: 0 }}
                transition={{ delay: 0.1 * index, duration: 0.3 }}
                className="bg-blue-50 p-2 rounded-lg text-gray-700"
              >
                {treatment}
              </motion.div>
            ))}
          ...
          <div className="grid grid-cols-2 gap-2">
            {info.prevention.map((prevention, index) => (
              <motion.div 
                ...
                animate={{ opacity: 1, x: 0 }}
                transition={{ delay: 0.1 * index, duration: 0.3 }}
                className="bg-green-50 p-2 rounded-lg text-gray-700"
              >
                {prevention}
              </motion.div>
            ))}
